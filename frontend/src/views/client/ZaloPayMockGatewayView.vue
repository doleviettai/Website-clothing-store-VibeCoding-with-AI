...
const handleConfirmPayment = async (status) => {
  ...
  statusMessage.value = status === 'SUCCESS' ? 'Đang xử lý thanh toán qua ZaloPay...' : 'Đang hủy giao dịch...'
  try {
    await zalopayApi.confirmMockZaloPay(orderId.value, appTransId.value, status)
    if (status === 'SUCCESS') {
...
<template>
  <div class="zalopay-gateway-mock min-vh-100 bg-light d-flex align-items-center justify-content-center py-5">
    <div class="card border-0 shadow-lg rounded-lg overflow-hidden" style="max-width: 480px; width: 100%;">
      <!-- Header ZaloPay Banner -->
      <div class="card-header text-center py-4" style="background: linear-gradient(135deg, #0068ff 0%, #0046b8 100%);">
        ...
        <h5 class="text-white font-weight-bold m-0">VÍ ĐIỆN TỬ ZALOPAY GATEWAY</h5>
      ...
      <div class="card-body p-4 bg-white text-center">
        ...
        <div v-else-if="isSuccessScreen" class="py-3">
          ...
          <p class="text-muted small mb-3">Giao dịch ZaloPay đã được xác nhận thành công.</p>
        ...
        <div v-else>
          <!-- Thông Tin Số Tiền -->
          <div class="p-3 rounded mb-4" style="background: #f0f7ff; border: 1px dashed #0068ff;">
          ...
          <div class="text-start small border rounded p-3 mb-4 bg-light">
            ...
            <div class="d-flex justify-content-between mb-2">
              <span class="text-muted">Mã ZaloPay TransID:</span>
          ...
          <div class="qr-box text-center mb-4">
            <div class="d-inline-block border p-2 rounded bg-white shadow-sm">
              <img
                src="https://api.qrserver.com/v1/create-qr-code/?size=180x180&data=ZaloPaySandboxDemo"
                alt="QR Code ZaloPay Sandbox"
                style="width: 160px; height: 160px;"
              />
            </div>
            <small class="text-muted d-block mt-2">Bấm nút bên dưới để xác nhận thanh toán qua Ví ZaloPay</small>
          </div>
      ...
      <div class="card-footer bg-light text-center py-3 border-top">
        <small class="text-muted">Trang thanh toán ví điện tử ZaloPay Sandbox thử nghiệm.</small>
      </div>
    </div>
  </div>
</template>
